...
def handle_auth_input(message):
    try:
        ...
        if isinstance(state, dict) and state.get('step') == 'login_password':
            login = state['login']
            ...
            if authenticated_user:
                ...
                # Store authentication state in user_states
                user_states[user_id] = {
                    'login': login,
                    'user_type': user_type,
                        ...
                        if profile_complete:
                            ...
                            cursor.execute("""
                                SELECT lname, fname, mname, citizenship, nationality, 
                                    date_of_birth, gender, phone_number, IIN, year_of_college 
                                FROM student 
                                WHERE student_id = %s
                            """, (user_type_id,))
                            ...
                            profile_message = (
                                ...
                                f"ИИН: {record[8]}\n"
                                f"Год обучения: {record[9]}"
                            )
                            
                            # Create a markup for confirmation
                            ...
                            # Set a state for profile confirmation
                            user_auth_state[user_id]['profile_confirmation'] = True
                        else:
...
def handle_message(message):
    ...
    if user_id not in user_states or user_states[user_id].get('user_type') != 'student':
        bot.send_message(user_id, "Пожалуйста, сначала войдите в систему.")
        return

    student_id = user_states[user_id]['user_type_id']
    ...
    if step == 'last_name':
        ...
        bot.send_message(user_id, "Теперь, пожалуйста, введите курс обучения(1, 2, 3):")

    elif step == 'IIN':
        student_data['IIN'] = message.text
        current_step[user_id] = 'year_of_college'
        bot.send_message(user_id, "Введите год обучения (1, 2 или 3):")
        ...
        if message.text in ['1', '2', '3']:
            ...
            bot.send_message(user_id, "Пожалуйста, введите 1, 2 или 3.")

@bot.message_handler(func=lambda message: message.chat.id in user_auth_state and user_auth_state[message.chat.id].get('profile_confirmation', False))
def handle_profile_confirmation(message):
    user_id = message.chat.id
    
    if message.text == "Да":
        # Reset profile confirmation state
        user_auth_state[user_id]['profile_confirmation'] = False
        
        # Start profile update process
        current_step[user_id] = 'last_name'
        bot.send_message(user_id, "Введите вашу Фамилию:", reply_markup=types.ReplyKeyboardRemove())
    
    elif message.text == "Нет":
        # Clear profile confirmation state
        user_auth_state[user_id]['profile_confirmation'] = False
        bot.send_message(user_id, "Хорошо, ваши данные остались прежними.", reply_markup=types.ReplyKeyboardRemove())
...
def read_data(message):
    if message.chat.id not in user_auth_state:
        bot.send_message(message.chat.id, "Пожалуйста, сначала войдите в систему через /login.")
        return
