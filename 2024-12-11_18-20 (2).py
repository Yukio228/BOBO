...
bot = telebot.TeleBot('7752765418:AAFkncYZFdwfjdRg2MGdrpS0uvnDR00wE5I')
...
def export_student_data(user_id, role='admin', extra=None, filter_citizenship='РК', **filters):
    try:
        ...
        # Добавление фильтров из переданных аргументов
        if filters.get('filter_group'):
            base_query += " AND g.group_name = %s"
            params.append(filters['filter_group'])
        
        if filters.get('filter_year'):
            base_query += " AND s.year_of_college = %s"
            params.append(filters['filter_year'])
        
        if filters.get('filter_citizenship_rk') == 'РК':
            base_query += " AND s.citizenship = 'РК'"
        elif filters.get('filter_citizenship_international') == 'international':
            base_query += " AND s.citizenship != 'РК'"
        
        if filters.get('filter_family_status'):
            base_query += """ 
                LEFT JOIN family_member f ON s.student_id = f.student_id
                AND f.family_type = %s
            """
            params.append(filters['filter_family_status'])
        
        if filters.get('filter_age_range'):
            age_filter = filters['filter_age_range']
            if age_filter == 'under_15':
            ...
            elif age_filter == 'over_18':
                base_query += " AND (EXTRACT(YEAR FROM AGE(s.date_of_birth)) > 18)"
        
        if filters.get('filter_gender'):
            base_query += " AND s.gender = %s"
            params.append('м' if filters['filter_gender'] == 'male' else 'ж')
        ...
        # Определяем список столбцов для DataFrame
        columns = ['lname', 'fname', 'mname', 'group_name', 'citizenship', 'reason_for_stay', 
                   'nationality', 'address_constant', 'address_home', 'date_of_birth', 'gender', 
                   'phone_number', 'IIN', 'year_of_college']
        
        # Если применен фильтр по семейному статусу, добавляем соответствующие столбцы
        if filters.get('filter_family_status'):
            columns.extend(['family_type', 'member_type', 'name', 'job', 'phone_number'])
        ...
        if filters.get('filter_group'):
            filename_parts.append(f"группа_{filters['filter_group']}")
        if filters.get('filter_year'):
            filename_parts.append(f"{filters['filter_year']}_курс")
        if filters.get('filter_citizenship'):
            filename_parts.append(f"гражданство_{filters['filter_citizenship']}")
        if filters.get('filter_family_status'):
            filename_parts.append(f"семья_{filters['filter_family_status']}")
        if filters.get('filter_age_range'):
            filename_parts.append(f"возраст_{filters['filter_age_range']}")
        if filters.get('filter_gender'):
            filename_parts.append(f"пол_{filters['filter_gender']}")
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('select_filter_'))
def handle_filter_selection(call):
    user_id = call.from_user.id
    
    # Инициализируем словарь фильтров для пользователя, если его нет
    ...
    if filter_type == 'specific_group':
        bot.send_message(user_id, "Введите название группы:")
        bot.register_next_step_handler_by_chat_id(user_id, process_group_selection)
        return
    
    # Логика мультивыбора для простых фильтров
    if filter_type in ['all_groups', 'year_1', 'year_2', 'year_3', 
                       'citizenship', 'family_status', 
                       'age', 'gender_male', 'gender_female']:
        
        # Для взаимоисключающих фильтров
        if filter_type.startswith('year_'):
            # Удаляем другие фильтры курсов
            user_filters[user_id] = {k: v for k, v in user_filters[user_id].items() if not k.startswith('year_')}
        
        if filter_type.startswith('gender_'):
            # Удаляем другие фильтры пола
            user_filters[user_id] = {k: v for k, v in user_filters[user_id].items() if not k.startswith('gender_')}
        
        # Переключение выбора фильтра
        if filter_type in user_filters[user_id]:
            del user_filters[user_id][filter_type]
        else:
            user_filters[user_id][filter_type] = True
        
        # Обновляем клавиатуру
        bot.edit_message_reply_markup(
            chat_id=user_id, 
            message_id=call.message.message_id, 
            reply_markup=create_admin_multifilter_keyboard(user_id)
        )
...
def process_group_selection(message):
    ...
    cursor.execute("SELECT group_name FROM groups WHERE group_name = %s", (group_name,))
    result = cursor.fetchone()
    
    if result:
        user_filters[user_id]['specific_group'] = group_name
        bot.send_message(user_id, f"Выбрана группа: {group_name}")
    else:
        bot.send_message(user_id, "Группа не найдена. Попробуйте еще раз.")


@bot.callback_query_handler(func=lambda call: call.data == 'apply_filters')
def apply_selected_filters(call):
    user_id = call.from_user.id
    ...
    export_student_data(
        user_id=user_id, 
        user_type='admin',  # Подставьте нужный тип
        login=None,  # Подставьте логин
...
# В обработчике callback-запроса
    elif call.data == "select_filter_all_groups":
