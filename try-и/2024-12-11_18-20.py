...
def reset_user_filters(user_id):
    """Сброс фильтров для пользователя"""
...
def toggle_filter(user_id, filter_type, filter_value):
    """Переключение фильтра (добавление/удаление)"""
    if user_id not in user_filters:
        reset_user_filters(user_id)
    
    if filter_value in user_filters[user_id][filter_type]:
        user_filters[user_id][filter_type].remove(filter_value)
    else:
        user_filters[user_id][filter_type].add(filter_value)

def create_multi_filter_keyboard(user_id, filter_type):
    """Создание клавиатуры с выбранными фильтрами"""
    keyboard = []
    
    # Определение опций в зависимости от типа фильтра
    if filter_type == 'gender':
        options = [
            ('export_gender_male', 'Мужской'),
            ('export_gender_female', 'Женский')
        ]
    elif filter_type == 'age_range':
        options = [
            ('age_under_15', 'До 15 лет'),
            ('age_15', '15 лет'),
            ('age_16', '16 лет'),
            ('age_17', '17 лет'),
            ('age_18', '18 лет'),
            ('age_over_18', 'Старше 18')
        ]
    elif filter_type == 'family_status':
        options = [
            ('family_full_admin', 'Полная семья'),
            ('family_incomplete_admin', 'Неполная семья'),
            ('family_orphan_admin', 'Сирота')
        ]
    elif filter_type == 'citizenship':
        options = [
            ('citizenship_rk_admin', 'РК'),
            ('citizenship_international_admin', 'Международное')
        ]
    else:
        return None
    
    for callback, text in options:
        # Проверяем, выбран ли текущий фильтр
        value = callback.split('_')[-1]
        is_selected = value in user_filters.get(user_id, {}).get(filter_type, set())
        
        # Добавляем emoji для визуальной индикации выбора
        display_text = f"{'✅ ' if is_selected else ''}{text}"
    ...
    keyboard.append([InlineKeyboardButton(
        "Применить фильтры", 
        callback_data=f"apply_filters_{filter_type}"
    )])
...
def handle_filter_callback(call):
    ...
    # Сохраняем существующую логику для admin_export
    ...
    if call.data.startswith("toggle_"):
        filter_data = call.data.split("toggle_", 1)[1]
        filter_type = None
        filter_value = None
        
        # Определение типа и значения фильтра
        if filter_data.startswith("export_gender_"):
            filter_type = 'gender'
            filter_value = filter_data.split("export_gender_", 1)[1]
        elif filter_data.startswith("age_"):
            filter_type = 'age_range'
            filter_value = filter_data
        elif filter_data.startswith("family_"):
            filter_type = 'family_status'
            filter_value = filter_data.split("family_", 1)[1].split("_admin")[0]
        elif filter_data.startswith("citizenship_"):
            filter_type = 'citizenship'
            filter_value = filter_data.split("citizenship_", 1)[1].split("_admin")[0]
        
        if filter_type and filter_value:
            toggle_filter(user_id, filter_type, filter_value)
            ...
            bot.edit_message_text(
                ...
                chat_id=user_id, 
                message_id=call.message.message_id, 
                reply_markup=create_multi_filter_keyboard(user_id, filter_type)
            )
    
    # Применение фильтров
    elif call.data.startswith("apply_filters_"):
        filter_type = call.data.split("apply_filters_", 1)[1]
        
        # Логика применения фильтров
        if user_filters.get(user_id, {}).get(filter_type):
            export_student_data(
                user_id, 
                'admin', 
                filters={
                    filter_type: list(user_filters[user_id][filter_type])
                }
            )
            
            # Сброс фильтров после применения
            reset_user_filters(user_id)
            
            bot.send_message(
                user_id, 
                "Данные экспортированы с выбранными фильтрами!"
            )

def create_admin_main_keyboard():
    """Создание главной клавиатуры для администратора с новыми фильтрами"""
    keyboard = [
        [InlineKeyboardButton("Фильтр по полу", callback_data="filter_gender")],
        [InlineKeyboardButton("Фильтр по возрасту", callback_data="filter_age_range")],
        [InlineKeyboardButton("Фильтр по семейному статусу", callback_data="filter_family_status")],
        [InlineKeyboardButton("Фильтр по гражданству", callback_data="filter_citizenship")],
        # Добавьте другие существующие кнопки
    ]
    return InlineKeyboardMarkup(keyboard)

def export_student_data(user_id, role, filters=None):
    """Модифицированная функция экспорта с поддержкой фильтров"""
    if not filters:
        # Если фильтры не указаны, экспортируем всех
        return export_all_students()
    
    # Логика фильтрации (зависит от вашей реализации базы данных)
    filtered_students = []
    
    # Применение фильтров
    for key, values in filters.items():
        if key == 'gender':
            filtered_students.extend([
                student for student in get_students() 
                if student['gender'] in values
            ])
        elif key == 'age_range':
            filtered_students.extend([
                student for student in get_students() 
                if any(str(student['age_range']) == v for v in values)
            ])
        elif key == 'family_status':
            filtered_students.extend([
                student for student in get_students() 
                if student['family_status'] in values
            ])
        elif key == 'citizenship':
            filtered_students.extend([
                student for student in get_students() 
                if student['citizenship'] in values
            ])
    
    # Экспорт отфильтрованных студентов
    export_students_to_file(filtered_students)

# Дополнительный обработчик для переключения на фильтры
def handle_filter_selection(call):
    user_id = call.from_user.id
    
    if call.data == "filter_gender":
        bot.edit_message_text(
            "Выберите пол:", 
            chat_id=user_id, 
            message_id=call.message.message_id, 
            reply_markup=create_multi_filter_keyboard(user_id, 'gender')
        )
    elif call.data == "filter_age_range":
        bot.edit_message_text(
            "Выберите возраст:", 
            chat_id=user_id, 
            message_id=call.message.message_id, 
            reply_markup=create_multi_filter_keyboard(user_id, 'age_range')
        )
    elif call.data == "filter_family_status":
        bot.edit_message_text(
            "Выберите семейный статус:", 
            chat_id=user_id, 
            message_id=call.message.message_id, 
            reply_markup=create_multi_filter_keyboard(user_id, 'family_status')
        )
    elif call.data == "filter_citizenship":
        bot.edit_message_text(
            "Выберите гражданство:", 
            chat_id=user_id, 
            message_id=call.message.message_id, 
            reply_markup=create_multi_filter_keyboard(user_id, 'citizenship')
        )
