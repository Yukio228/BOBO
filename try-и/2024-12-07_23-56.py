...
def export_student_data(user_id, user_type, login, filter_group=None, filter_year=None, 
                        filter_citizenship=None, filter_family_status=None, 
                        filter_age_range=None, filter_gender=None):
    try:
        # Base query for student data export
        base_query = """
        ...
        if filter_group:
            filename_parts.append(f"group_{filter_group}")
        if filter_year:
            filename_parts.append(f"year_{filter_year}")
        if filter_citizenship:
            filename_parts.append(f"citizenship_{filter_citizenship}")
        if filter_family_status:
            filename_parts.append(f"family_{filter_family_status}")
        if filter_age_range:
            filename_parts.append(f"age_{filter_age_range}")
        if filter_gender:
            filename_parts.append(f"gender_{filter_gender}")
...
def create_admin_main_keyboard():
    ...
    keyboard.row(
        types.InlineKeyboardButton("Все группы", callback_data="export_all_groups"),
